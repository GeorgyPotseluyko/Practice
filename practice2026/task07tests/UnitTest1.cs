...
public class AttributeReflectionTests
{
    ...
    {
        var type = typeof(SampleClass);
        var info = ReflectionHelper.PrintTypeInfo(type);
        Assert.Contains("DisplayName: Пример класса", info);
        Assert.Contains("Version: 1.0", info);
        Assert.Contains("Свойство: Number, DisplayName: Числовое свойство", info);
        Assert.Contains("Метод: TestMethod, DisplayName: Тестовый метод", info);
    }
}